...
WinUI app (`HaCompanion.App`) hosts the Home Assistant frontend in a `WebView2`,
adds a system tray icon, shows native toast notifications, and wires OS power
events. Authentication for the MVP uses a long-lived access token; the PC registers
as a `mobile_app` device and reports battery-level and battery-state sensors.
...
`Microsoft.Web.WebView2`, `System.Text.Json`, `Microsoft.Windows.SDK.BuildTools`,
`System.Net.WebSockets.Client` (BCL). Tray icon via `H.NotifyIcon.WinUI`. Toasts
...
for the token and webhook secret; a small JSON app-settings file
...
**Project Type**: Desktop application (native shell + embedded web view) with a
reusable core library.

**Performance Goals**: Dashboard visible within ~2s of a warm launch; sensor update
CPU cost negligible (<1% average); toast latency <10s while connected.
...
- **I. Native Windows Experience First**: PASS — WinUI 3 shell, Fluent theme,
  system tray, native toasts; web content confined to WebView2.
...
└── HaCompanion.App/             # WinUI 3 (Windows App SDK) desktop app
    ├── App.xaml(.cs)            # App bootstrap, single-instance, tray lifecycle
    ├── MainWindow.xaml(.cs)     # WebView2 host + status bar
    ├── Views/                   # ConnectView (URL + token entry)
    ├── Services/                # WindowsSecretStore, WindowsSystemStatusProvider,
    │                            # ToastNotifier, TrayIconService
    └── Assets/
...
1. **Auth (MVP)**: Long-lived access token entered by the user. Sent as
   `Authorization: Bearer <token>` for REST/WebSocket. OAuth2 IndieAuth is deferred.
...
4. **Notifications**: Maintain a WebSocket connection (`/api/websocket`), authenticate
   with the token, and subscribe to `persistent_notification` creation events (and/or
   a configured event) to render Windows toasts. This replaces mobile push, which
   Windows lacks an equivalent for. Documented as an MVP assumption.
5. **Dashboard**: `WebView2` navigates to the Home Assistant base URL. To avoid a
   second interactive login inside the web view, inject the long-lived token into
   `localStorage` as `hassTokens` before navigation (the same mechanism HA's own
   apps use), falling back to normal web login if injection is unavailable.
...
7. **Security**: `WindowsSecretStore` wraps `PasswordVault`. Logging uses a
   redaction layer; DTOs never `ToString()` secrets.
